...
board_t * makeBoard(int w, int h, int numMines) {
  // Allocate memory for the board_t structure
  board_t * b = malloc(sizeof(*b));
  if (b == NULL) {
    perror("Failed to allocate memory for board_t");
    exit(EXIT_FAILURE);
  }

  // Initialize the width, height, and totalMines fields
  b->width = w;
  b->height = h;
  ...
  // Initialize all squares on the board to UNKNOWN
  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {
      b->board[y][x] = UNKNOWN;
    }
...
int countMines(board_t * b, int x, int y) {
  ...
  // Loop through the 8 surrounding squares
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      // Skip the center square (x, y)
      if (dx == 0 && dy == 0) {
        continue;
      }
...
int checkWin(board_t * b) {
  for (int y = 0; y < b->height; y++) {
    for (int x = 0; x < b->width; x++) {
      if (b->board[y][x] == UNKNOWN) {
        return 0; // Game is not won if any square is still UNKNOWN
      }
    }
  }
  return 1; // All squares are revealed or marked as mines
}
...
void freeBoard(board_t * b) {
  ...
  free(b->board);
  // Free the board_t structure itself
  free(b);
}
